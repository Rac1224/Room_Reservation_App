...
{
    public partial class frmStudentNewReservation : Form
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["systemDB"].ToString());
        //Student ID and Name currently assessing the form
        public static string id;
        public static string name;
        public frmStudentNewReservation(string studentID, string studentName)
        {
            InitializeComponent();
            id = studentID;
            name = studentName;
        ...
        private void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //create new object
                reservation newReservation = new reservation(selectedRoom, date, startTime, endTime, id, name, status);
                //call method 
                reservationSuccess = newReservation.studentCreateNewReservation(reservationSuccess, newReservation.Room, newReservation.Date, newReservation.StartTime, newReservation.EndTime,
                    newReservation.StudentID, newReservation.StudentName, newReservation.Status);
                if (reservationSuccess == true)
                    this.Close(); // if new reservation is made successfully. close this form
            }

            //catches any exception, for e.g: nullreferenceexception if user did not select a particular option
            catch(Exception)
            {
                MessageBox.Show("Error Occured!\nPlease select valid room, date, and time for your reservation details!",
                                    "Reservation Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

        }

        private void frmStudentNewReservation_Load(object sender, EventArgs e)
        {
        ...
        private void frmStudentNewReservation_FormClosed(object sender, FormClosedEventArgs e)
        {
            //return to Student Functionality form
            this.Hide();
